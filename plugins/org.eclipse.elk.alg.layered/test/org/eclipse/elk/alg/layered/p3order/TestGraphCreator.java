 ...
 * that the methods for creating edges cannot be used in every case. TODO consider moving all
 * downward into base
 * 
 * @author alan
 *
...
public class TestGraphCreator {
    ...
    public LNode[][] getCurrentOrder(final LGraph graph) {
    ...
    public void addNorthSouthEdge(final PortSide side, final LNode nodeWithNSPorts,
    ...
    public Layer[] makeLayers(final int amount, final LGraph graph) {
        Layer[] layers = new Layer[amount];
        for (int i = 0; i < layers.length; i++) {
            layers[i] = makeLayer(graph);
        }
        return layers;
    }
    ...
    protected List<LNode> switchOrderOfNodesInLayer(final int nodeOne, final int nodeTwo,
